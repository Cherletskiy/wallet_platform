...
from auth_service.infrastructure.sa.migrations import run_migrations
from auth_service.infrastructure.sa.session import close_db, init_db
from auth_service.ioc import MainProvider
from auth_service.presentation.api import register_exception_handlers, routers
...
def create_app(
    ...
    *,
    ...
    app = FastAPI(
        ...
        description="API for registration and JWT-based authentication",
        version="1.0.0",
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="auth-service",
        description="Run the auth service application.",
    )
    parser.add_argument(
        "--host",
    ...
    parser.add_argument(
        "--port",
        type=int,
        default=int(os.getenv("APP_PORT", "8002")),
