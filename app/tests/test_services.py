...
from app.models.wallet import Wallet, OperationType
...
async def test_get_wallet_balance_rub_success(wallet_service: WalletService, mock_wallet_repository):
...
async def test_get_wallet_balance_rub_not_found(wallet_service: WalletService, mock_wallet_repository):
...
async def test_update_wallet_balance_deposit_success(wallet_service: WalletService, mock_wallet_repository):
    ...
    balance = await wallet_service.update_wallet_balance_cent(mock_session, wallet_id, 5000, OperationType.DEPOSIT)
...
async def test_update_wallet_balance_withdrawal_success(wallet_service: WalletService, mock_wallet_repository):
    ...
    balance = await wallet_service.update_wallet_balance_cent(mock_session, wallet_id, 5000, OperationType.WITHDRAWAL)
...
async def test_update_wallet_balance_withdrawal_insufficient(wallet_service: WalletService, mock_wallet_repository):
    ...
    with pytest.raises(HTTPException) as exc:
        await wallet_service.update_wallet_balance_cent(mock_session, wallet_id, 5000, OperationType.WITHDRAWAL)

    assert exc.value.status_code == 400
    assert exc.value.detail == "Not enough balance"


async def test_update_wallet_balance_invalid_amount(wallet_service: WalletService, mock_wallet_repository):
    ...
    with pytest.raises(HTTPException) as exc:
        await wallet_service.update_wallet_balance_cent(mock_session, wallet_id, -5000, OperationType.DEPOSIT)

    assert exc.value.status_code == 400
    assert exc.value.detail == "Amount must be greater than 0"


async def test_get_wallet_balance_rub_repository_exception(wallet_service: WalletService, mock_wallet_repository):
    ...
    mock_wallet_repository.get_wallet_balance_cent = AsyncMock(
        side_effect=HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Repository error")
    )
...
async def test_update_wallet_balance_cent_general_exception(wallet_service: WalletService, mock_wallet_repository):
    """Тест обработки общей ошибки в update_wallet_balance_cent."""
    wallet_id = uuid.uuid4()
    mock_wallet = Wallet(id=wallet_id, balance_cent=10000)
    mock_wallet_repository.get_wallet_by_id = AsyncMock(return_value=mock_wallet)
    mock_wallet_repository.update_wallet_balance_cent = AsyncMock(side_effect=Exception("Database failure"))
    mock_wallet_repository.add_operation = AsyncMock(return_value=None)
