...
class WalletService:
    ...
    async def get_wallet_balance_rub(
        ...
        """
        try:
            balance_cent = await self.wallet_repository.get_wallet_balance_cent(session, wallet_id)
            ...
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Error getting wallet balance")
    ...
    async def update_wallet_balance_cent(
        ...
        if operation_type == OperationType.WITHDRAWAL and wallet.balance_cent < amount_cent:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Not enough balance")

        balance = wallet.balance_cent
        new_balance_cent = balance - amount_cent if operation_type == OperationType.WITHDRAWAL \
            else balance + amount_cent

        try:
            await self.wallet_repository.update_wallet_balance_cent(session, wallet_id, new_balance_cent)
            await self.wallet_repository.add_operation(session, wallet_id, operation_type, amount_cent)
            await session.commit()
            logger.info(f"Updated wallet balance id: {wallet_id}. Operation type: {operation_type}. "
                        f"Balance_cent before: {balance}, after: {new_balance_cent}")
        except ValueError:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Wallet not found")
        except Exception as e:
            logger.error(f"Error updating wallet balance {wallet_id}. "
                         f"Balance before: {balance}, after: {new_balance_cent}. Error: {e}")
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Error updating wallet balance")
